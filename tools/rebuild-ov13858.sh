...
#
# ПОЧЕМУ НЕ В ipu-bridge, ГДЕ ДЛЯ ЭТОГО ЕСТЬ ТАБЛИЦА. Пробовал — не выходит:
# intel_ipu6 линкуется с символами ipu_bridge_init и ipu_bridge_parse_ssdb, а
# сборка ipu-bridge вне дерева даёт другие контрольные суммы, и ядро отвергает
# модуль:
#   intel_ipu6: disagrees about version of symbol ipu_bridge_init
# Пересобирать заодно и intel_ipu6 ради одной строки таблицы не стоит, поэтому
# правка сделана в самом драйвере сенсора — он листовой, наружу ничего не
# экспортирует. Ровно так же поступает вторая половина PR #2227.
#
# Использование:
