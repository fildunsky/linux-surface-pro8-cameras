...
# и регистрация валится с -E2BIG: ядро именно отказывает, а не молча обрезает.
#
# ПОПРАВКА к прежней редакции этого комментария. Здесь стояло, что поднять
# константу нельзя, раз она задаёт размер поля supply_name_upper в структуре
# int3472_discrete_device, общей с модулем intel_skl_int3472_common. Это
# неверно, и я успел сказать это же в linux-surface#2252, где на моих словах
# правили документацию по миграции. Структуру видят только discrete.c,
# clk_and_regulator.c и led.c, а Makefile собирает все три в один модуль,
# intel_skl_int3472_discrete; в common.c и tps68470.c нет ни одной ссылки на
# неё. Границу модуля структура не пересекает, поднимать константу безопасно.
#
# Мы всё равно её не поднимаем, но по другой причине: она объявлена в
# common.h, а этот скрипт правит только .c-файлы, и проверять правильнее то,
# что действительно упирается в предел, — итоговое имя регулятора
...
NEW_CHK = """	/*
	 ...
	 * Строку в верхнем регистре держим в отдельной памяти, чтобы не
	 * упираться в GPIO_SUPPLY_NAME_LENGTH: константа живёт в common.h,
	 * который мы не правим.
	 */
